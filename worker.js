...
<body>
  <main>
    <section id="create-view">
      <div>
        <h1>Steam Guard Link</h1>
        <p class="sub">浏览器本地加密，生成 /密文#钥匙 分享链接。</p>
      </div>
      <section class="panel">
        ...
        <div id="result" class="result">
          ...
          <p class="note">Worker 不存储密钥，也拿不到 # 后面的解密钥。完整链接本身就是访问凭证。</p>
        </div>
      </section>
    </section>
  ...
  <script nonce="__NONCE__">
    ...
    const state = { payload: "", key: "", token: "", timer: 0, clockOffsetMs: 0, period: 30 };
    ...
    createButton.addEventListener("click", async () => {
      ...
      try {
        ...
        linkOutput.value = location.origin + "/" + base64UrlEncode(packed) + "#" + base64UrlEncode(rawKey);
    ...
    async function openSharedLink() {
      try {
        state.payload = location.pathname.slice(1);
        state.key = location.hash.replace(/^#/, "");
        if (!state.payload || !state.key) throw new Error("链接缺少解密钥，请复制完整链接。");
        await fetchToken();
      } catch (error) {
    ...
    async function fetchToken() {
      try {
        ...
        const response = await fetch("/api/token", {
          ...
          cache: "no-store",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ payload: state.payload, key: state.key })
        });
